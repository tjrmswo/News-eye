...
export default function AnalystPage() {
  ...
  // ✅ localStorage를 state로 관리
  const [localStorageAnalysisField, setLocalStorageAnalysisField] =
    useState<string>("");
  ...
  useEffect(() => {
    const savedField = localStorage.getItem("analysisField") || "";
    setLocalStorageAnalysisField(savedField);

    // 초기 메시지 설정
    const displayField = analysisField.length > 0 ? analysisField : savedField;
    setAnalysisResults([
      `AI로 ${displayField} 분야를 분석한 결과를 알려드리겠습니다!`,
    ]);
  }, [analysisField]);
    ...
    <div className="flex h-full flex-col rounded-xl bg-white p-7">
      <div className="flex flex-row justify-between">
        <span className="text-lg">
          {/* ✅ state 사용 */}
          {localStorageAnalysisField} 분야 분석 결과
        </span>
      </div>
